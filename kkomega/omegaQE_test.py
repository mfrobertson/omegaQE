...
def get_F_L_and_C_inv_splines(fields, Lmax=5000):
    fish = Fisher()
    sample_Ls = fish.covariance.get_log_sample_Ls(Lmin=2, Lmax=Lmax, Nells=300)
...
plt.loglog(omega_Ls, (2 * np.pi) ** 2 * omega_ps)
plt.loglog(omega_Ls, (2 * np.pi) ** 2 * omega_ps/F_L_spline(omega_Ls), label="Fiducial")
plt.xlim(3e1, 4e3)
plt.ylim(1e-12,1e-5)
plt.legend()
plt.show()
