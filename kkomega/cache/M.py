...
def M_matrix(mode, ells, star):
    Nells = np.size(ells)
    M = np.ones((Nells, Nells))
    for iii, ell in enumerate(ells):
        M[iii, :] = mode.components(np.ones(Nells) * ell, ells, star)
    return M


def save(ells, M, star):
...
def main(ellmax, Nells, star):
    mode = Modecoupling()
    ells = mode.generate_sample_ells(ellmax, Nells)
    M = M_matrix(mode, ells, star)
    save(ells, M, star)
...
if __name__ == "__main__":
    args = sys.argv[1:]
    if len(args) != 3:
        print("Must supply arguments: ellmax Nells star")
    ellmax = int(args[0])
    Nells = int(args[1])
    star = tools.parse_boolean(args[2])
    main(ellmax, Nells, star)
