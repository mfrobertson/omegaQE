...
class Bias:
    ...
    def bias(self, XY, Ls, N_L1=20, N_L3=20, Ntheta12=50, Ntheta13=10):
        A = self._qe.normalisation(XY, Ls, curl=True)
        print(f"A: {A}")
        ...
        for lll, L in enumerate(Ls):
            ...
            for iii, L1 in enumerate(Ls1):
                ...
                for jjj, theta1 in enumerate(thetas1):
                    ...
                    bi = w2 * self.mixed_bispectrum("theory", L1, L2, L1_vec.deltaphi(L2_vec))
                    ...
                    if w2 != 0:
                        for kkk, L3 in enumerate(Ls3):
                            ...
                            g_XY = self._qe.weight_function(XY, L_vec, L3_vec, curl=True)
                            g_YX = self._qe.weight_function(XY[::-1], L_vec, L3_vec, curl=True)
                            L4_vec = L_vec - L3_vec
            ...
            A1[lll] = -A[lll] / ((2 * np.pi) ** 4) * InterpolatedUnivariateSpline(Ls1, I_A1_L1).integral(30, 4000)
            C1[lll] = 0.5*A[lll] / ((2 * np.pi) ** 4) * InterpolatedUnivariateSpline(Ls1, I_C1_L1).integral(30, 4000)
        return A1, C1

    def bias_phi(self, XY, Ls, N_L1=20, N_L3=20, Ntheta12=50, Ntheta13=10):
        A = self._qe.normalisation(XY, Ls, curl=False)
        print(f"A: {A}")
        samp1_1 = np.arange(30, 40, 5)
        samp2_1 = np.logspace(1, 3, N_L1) * 4
        Ls1 = np.concatenate((samp1_1, samp2_1))
        ...
        for lll, L in enumerate(Ls):
            ...
            for iii, L1 in enumerate(Ls1):
                ...
                for jjj, theta1 in enumerate(thetas1):
                    ...
                    L2 = L2_vec.rho
                    w2 = 0 if (L2 < 3 or L2 > 4000) else 1
                    bi = w2 * self.mixed_bispectrum("conv", L1, L2, L1_vec.deltaphi(L2_vec))
                    I_A1_L3 = np.zeros(np.size(Ls3))
                    I_C1_L3 = np.zeros(np.size(Ls3))
                    if w2 != 0:
                        for kkk, L3 in enumerate(Ls3):
                            ...
                            L_A1_fac = (L5_vec @ L1_vec) * (L5_vec @ L2_vec)
                            L_C1_fac = (L3_vec @ L1_vec) * (L3_vec @ L2_vec)
                            g_XY = self._qe.weight_function(XY, L_vec, L3_vec, curl=False)
                            g_YX = self._qe.weight_function(XY[::-1], L_vec, L3_vec, curl=False)
                            L4_vec = L_vec - L3_vec
                            h_X_A1 = self._qe.geo_fac(XY[0], theta12=L5_vec.deltaphi(L4_vec))
                            h_Y_A1 = self._qe.geo_fac(XY[1], theta12=L5_vec.deltaphi(L3_vec))
                            h_X_C1 = self._qe.geo_fac(XY[0], theta12=L3_vec.deltaphi(L4_vec))
