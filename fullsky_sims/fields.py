...
class Fields:
    ...
    def get_cached_cmb_lens_rec(self, typ, cmb_fields, sim=None, deflect_typ=None, iter=False, gmv=True, cmb_noise=True, bh=None):
        ...
        if bh is not None:
            ext += f"_bh{bh}"
        print(f"Getting cached {typ} reconstruction for sim: {sim}, deflection type: {deflect_typ}, exp: {self.exp}, iter: {iter}, gmv: {gmv}, and bias_hard: {bh}")
    ...
    def _get_cmb_lens_rec(self, typ, cmb_fields, iter, fft, gmv, cmb_noise, bh=None):
        if self.rec is None:
            map = self.get_cached_cmb_lens_rec(typ, cmb_fields, iter=iter, gmv=gmv, cmb_noise=cmb_noise, bh=bh)
            if fft:
    ...
    def get_kappa_rec(self, cmb_fields="T", iter=False, fft=False, gmv=True, cmb_noise=True, bh=None):
        return self._get_cmb_lens_rec("kappa", cmb_fields, iter, fft, gmv, cmb_noise, bh)

    def get_omega_rec(self, cmb_fields="T", iter=False, fft=False, gmv=True, cmb_noise=True, bh=None):
