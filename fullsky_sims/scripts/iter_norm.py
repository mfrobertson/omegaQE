...
def _save_norms(norm_k, norm_w, exp, qe_typ, offset, nbins_k, nbins_w):
    norm_dir_k = f"{fullsky_sims.CACHE_DIR}/_iter_norm/{exp}/{qe_typ}/{offset}_{nbins_k}"
    if not os.path.exists(norm_dir_k):
        os.makedirs(norm_dir_k)
    norm_dir_w = f"{fullsky_sims.CACHE_DIR}/_iter_norm/{exp}/{qe_typ}/{offset}_{nbins_w}"
...
def main(exp, qe_typ, offset, nbins_k, nbins_w, nthreads, _id):
    ...
    name_ext = "diff2"
    nsims = 40
    kappa_true = dm.sht.read_map(f"{fullsky_sims.SIMS_DIR}/kappa_{name_ext}.fits")
    omega_true = dm.sht.read_map(f"{fullsky_sims.SIMS_DIR}/omega_{name_ext}.fits")
    kappa_map = dm.sht.read_map(f"{fullsky_sims.SIMS_DIR}/{deflect_typ}/{exp}/kappa/{qe_typ}_iter_{0}.fits")
    omega_map = dm.sht.read_map(f"{fullsky_sims.SIMS_DIR}/{deflect_typ}/{exp}/omega/{qe_typ}_iter_{0}.fits")
    cl_k_cross = dm.sht.map2cl(kappa_true, kappa_map)
    cl_w_cross = dm.sht.map2cl(omega_true, omega_map)
    for sim in np.arange(1, nsims):
        kappa_map = dm.sht.read_map(f"{fullsky_sims.SIMS_DIR}/{deflect_typ}/{exp}/kappa/{qe_typ}_iter_{sim}.fits")
        omega_map = dm.sht.read_map(f"{fullsky_sims.SIMS_DIR}/{deflect_typ}/{exp}/omega/{qe_typ}_iter_{sim}.fits")
        cl_k_cross += dm.sht.map2cl(kappa_true, kappa_map)
        cl_w_cross += dm.sht.map2cl(omega_true, omega_map)
