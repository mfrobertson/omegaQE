...
def _save_norms(norm_k, norm_w, exp, qe_typ, offset, nbins_k, nbins_w, noise, gmv):
    norm_dir_k = f"{dm.cache_dir}/_iter_norm/{exp}/{qe_typ}/{offset}_{nbins_k}"
    if not os.path.exists(norm_dir_k):
        os.makedirs(norm_dir_k)
    norm_dir_w = f"{dm.cache_dir}/_iter_norm/{exp}/{qe_typ}/{offset}_{nbins_w}"
...
def main(exp, qe_typ, offset, nbins_k, nbins_w, noise, gmv, nthreads, _id):
    dm.sht.nthreads=nthreads
    mpi.output("-------------------------------------", 0, _id)
    mpi.output(f"exp: {exp}, qe_typ: {qe_typ}, noise: {noise}, gmv: {gmv}, nthreads: {nthreads}", 0, _id)
...
if __name__ == '__main__':
    ...
    if len(args) != 9:
        raise ValueError(
            "Must supply arguments: exp qe_typ offset nbins_k nbins_w noise gmv nthreads _id")
    exp = str(args[0])
    qe_typ = str(args[1])
    offset = int(args[2])
    nbins_k = int(args[3])
    nbins_w = int(args[4])
    noise = parse_boolean(args[5])
    gmv = parse_boolean(args[6])
    nthreads = int(args[7])
    _id = str(args[8])
    main(exp, qe_typ, offset, nbins_k, nbins_w, noise, gmv, nthreads, _id)
