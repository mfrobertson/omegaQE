...
class Demnunii:
    ...
    def get_snap(self, snap):
        sep = getFileSep()
        hdul = fits.open(f'{self.data_dir}{sep}SurfaceMassDensities{sep}SurfDensMap_snap_0{snap}.0.fits')
    ...
    #
    # def _bias(self, snap, typ="unity"):
    #     if typ == "unity":
    #         return 1
    #     if typ == "LSST":
    #         return 1 + (0.84 * self._get_z(snap))
    #
    # def get_gal_map(self, zmin=0, zmax=1100, bias="unity", chi_min=None, chi_max=None, use_chi=False):
    #     pixel_area = hp.pixelfunc.nside2pixarea(self.nside)
    #     npix = hp.nside2npix(self.nside)
    #     particle_mass = self.get_particle_mass()
    #     gal = np.zeros(npix)
    #     snaps = self.get_snaps_chi(chi_min, chi_max) if use_chi else self.get_snaps_z(zmin, zmax)
    #     for snap in snaps:
    #         gal += self._bias(snap, bias) * self.get_snap(snap) * particle_mass / pixel_area
    #     gal_bar = np.mean(gal)
    #     return gal / gal_bar - 1
    #
    # def _window(self, z, typ):
    #     if typ == "LSST":
    #         z0 = 0.311
    #         return 1/(2*z0) * (z/z0)**2 * np.exp(-z/z0) * self.cosmo.get_hubble(z)
    #
    # def _get_Ng(self, zmin=0, zmax=1100, window="LSST"):
    #     zs = np.linspace(zmin, zmax, 1000)
    #     dz = zs[1] - zs[0]
    #     return np.sum(self._window(zs, window)) * dz
    #
    # def get_obs_gal_map(self, zmin=0, zmax=1100, bias="LSST", window="LSST", chi_min=None, chi_max=None, use_chi=False):
    #     pixel_area = hp.pixelfunc.nside2pixarea(self.nside)
    #     npix = hp.nside2npix(self.nside)
    #     particle_mass = self.get_particle_mass()
    #     gal = np.zeros(npix)
    ...
    #     for snap in snaps:
    #         z = self._get_z(snap)
    #         gal += self._bias(snap, bias) * self._window(z, window) * self.get_snap(snap) * particle_mass / pixel_area
    #     gal_bar = np.mean(gal)
    #     return gal / gal_bar - 1
