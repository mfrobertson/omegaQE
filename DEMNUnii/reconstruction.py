...
class Reconstruction:
    ...
    class MyMapLib:
        ...
        def _get_map_eblm(self, idx):
            return hp.sphtfunc.map2alm_spin([self.maps[1], self.maps[2]], 2, lmax=self.lmax, mmax=self.lmax)
    ...
    def __init__(self, exp, lmin=30, lmax=3000):
        self.setup_env()
    ...
    def get_filters(self, indices, noise_cls=None):
        filts = [(utils.cli(self.cl_len[idx][:self.lmax + 1] + noise_cls[idx[0]][:self.lmax + 1])) if noise_cls is not None else
                utils.cli(self.cl_len[idx][:self.lmax + 1]) for idx in indices]
        for filt in filts:
            filt[:self.lmin] *= 0.
        return filts
    ...
    def get_phi_rec(self, typ, TQUmaps_filename):
        ...
        transfer_dict = {idx[0]:np.ones(self.lmax + 1) for idx in indices}
        ...
        sim_lib = self.MySimLib(maps_lib, self.dm.nside)
        weighted_maps_lib = filt_simple.library_fullsky_sepTP(os.path.join(self.temp, 'ivfs'), sim_lib, self.dm.nside, transfer_dict, self.cl_len, filters[0], filters[1], filters[2], cache=True)
        ...
        cl_weight['bb'] *= 0.
        qresp_lib = qresp.resp_lib_simple(os.path.join(self.temp, 'qresp'), self.lmax, cl_weight, self.cl_grad, filt_dict, self.lmax_map)
