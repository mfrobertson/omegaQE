...
def main(exp, Nchi, tracer_noise, nsims, kappa_rec, kappa_qe_typ, deflect_typ, gauss_lss, len_lss, nthreads, _id):
    mpi.output("-------------------------------------", 0, _id)
    mpi.output(f"exp: {exp}, Nchi: {Nchi}, nsim: {nsims}, tracer_noise: {tracer_noise}, kappa_rec: {kappa_rec}, kappa_qe_typ: {kappa_qe_typ}, deflect_typ: {deflect_typ}, gauss_lss: {gauss_lss}, len_lss: {len_lss}, nthreads: {nthreads}", 0, _id)
    ...
    for sim in range(nsims):
        ...
        fields = Fields(exp, use_lss_cache=True, use_cmb_cache=True, cmb_sim=sim, deflect_typ=deflect_typ, gauss_lss=gauss_lss, len_lss=len_lss, nthreads=nthreads)
...
if __name__ == '__main__':
    ...
    if len(args) != 11:
        raise ValueError(
            "Must supply arguments: exp Nchi tracer_noise nsims kappa_rec kappa_qe_typ deflect_typ gauss_lss len_lss nthreads _id")
    exp = str(args[0])
    Nchi = int(args[1])
    ...
    nthreads = int(args[9])
    _id = str(args[10])
    main(exp, Nchi, tracer_noise, nsims, kappa_rec, kappa_qe_typ, deflect_typ, gauss_lss, len_lss, nthreads, _id)
