...
def _get_ps(exp, tracer_fields, deflect_typ, tem_ext, nsims, iter_mc_corr, nthreads, qe_typ):
...
def main(exp, tracer_fields, tracer_noise, kappa_rec, qe_typ, nsims, deflect_typ, gauss_lss, len_lss, iter_mc_corr, nthreads, _id):
    mpi.output("-------------------------------------", 0, _id)
    ...
    for deflect_typ in deflect_typs:
        ...
        ps = _get_ps(exp, tracer_fields, deflect_typ, ext, nsims, iter_mc_corr, nthreads, qe_typ)
...
if __name__ == '__main__':
    ...
    if len(args) != 12:
        raise ValueError(
            "Must supply arguments: exp tracer_fields tracer_noise kappa_rec qe_typ nsims deflect_typ gauss_lss len_lss iter_mc_corr nthreads _id")
    exp = str(args[0])
    ...
    _id = str(args[11])
    main(exp, tracer_fields, tracer_noise, kappa_rec, qe_typ, nsims, deflect_typ, gauss_lss, len_lss, iter_mc_corr, nthreads, _id)
